...
export default function FeedPage() {
  ...
  function PostCard({ img }){
    ...
    const baseStyle = {background:'rgba(255,255,255,0.02)',borderRadius:10,overflow:'hidden',border:'1px solid rgba(255,255,255,0.03)',opacity: entered ? 1 : 0, transform: entered ? 'translateY(0)' : 'translateY(8px)', transition: 'opacity .28s ease, transform .28s ease'}
    return (
      <div style={baseStyle}>
        ...
        <img src={img.imageUrl} alt={img.prompt} style={{width:'100%',display:'block',transition:'transform .18s ease'}} onMouseEnter={e=>{ e.currentTarget.style.transform='scale(1.02)' }} onMouseLeave={e=>{ e.currentTarget.style.transform='none' }} />
